...
- [x] Pulido general de estilo (espaciado, tipografía, micro-detalles) — a partir de un mockup de venta que le gustó a la clienta, se adaptó el estilo (no los colores del mockup: se mantuvo el bordó/rosa real) en toda la app: Sidebar/Header, Panel, Stock, Clientes, Caja, Usuarios e Historial de ventas. Sidebar oscuro responsive con marca e ítems con ícono, tarjetas de estadística con número grande (Panel y Caja), avatares circulares con iniciales en toda fila que atribuye a una persona (clientes, usuarios, movimientos, ventas), punto de color por estado de stock, foco de formulario en acento en toda la app, encabezados de lista con botones consistentes. De paso se encontraron y corrigieron dos lugares más con el mismo bug de nombres vacíos para operador (`ventas/historial` y la exportación de ventas a Excel usaban `perfiles` en vez de `perfiles_publico`)
